...
def write_tour_to_img(coords, tour, img_file):
   ...
   padding = 20
   coords = [((x/1000) + padding, (y/1000) + padding) for (x, y) in coords]
   maxx, maxy = 0, 0
   for x, y in coords:
   ...
   for i in range(num_cities):
      ...
      x1, y1 = (coords[city_i])
      x2, y2 = (coords[city_j])
      #d.line((int((x1/1000)), int((y1/1000)), int((x2/1000)), int((y2/1000))), fill=(0, 0, 0))
      #d.text((int((x1/1000)) + 7, int((y1/1000)) - 5), str(i), font=font, fill=(32, 32, 32)) #32

   for x, y in coords:
      x, y = int((x/1000)), int((y/1000))
      d.ellipse((x - 5, y - 5, x + 5, y + 5), outline=(0, 0, 0),fill=(196, 196, 196)) #196
   del d
...
class Individual:
    ...
    def __repr__(self):
        ...
        return '<%s chromosome="%s" score=%s>' % (self.__class__.__name__,
...
class Environment:
    size = 0
    def __init__(self, population=None, size=100, maxgenerations=1000,newindividualrate=0.6,crossover_rate=0.80,mutation_rate=0.05):
        self.size = size
...
def main_run():
   ...
   ev = Environment()
   ev.run()   
   write_tour_to_img(coords, ev.minindividual.chromosome,"tsp_image.png")
